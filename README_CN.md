...
本仓库还包含两个独立扩展模块：`sub2api-monitor` 提供多实例运维监控、成本路由、事件通知和受控账号恢复；`trusted-pool-platform` 提供可信 Seat 的暂停换员、凭据批次、恢复治理与公开证据验证。两者都已经形成开发闭环，但各自仍有明确的生产门禁，不能仅根据功能存在推断为已经可无条件上线。

统一的能力边界、最近验证证据和分阶段计划见[项目能力盘点与交付路线图](docs/PROJECT_CAPABILITIES_AND_ROADMAP_CN.md)。
