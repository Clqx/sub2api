        ...
        <div class="max-h-64 overflow-y-auto">
          ...
          >
            ...
            <div class="space-y-1.5">
              ...
              <template v-else>
                <div v-if="subscription.group?.hourly_limit_usd" class="flex items-center gap-2">
                  <span class="w-8 flex-shrink-0 text-[10px] text-gray-500">{{
                    t('subscriptionProgress.hourly')
                  }}</span>
                  <div class="h-1.5 min-w-0 flex-1 rounded-full bg-gray-200 dark:bg-dark-600">
                    <div
                      class="h-1.5 rounded-full transition-all"
                      :class="getProgressBarClass(subscription.hourly_usage_usd ?? 0, subscription.group.hourly_limit_usd)"
                      :style="{ width: getProgressWidth(subscription.hourly_usage_usd ?? 0, subscription.group.hourly_limit_usd) }"
                    ></div>
                  </div>
                  <span class="w-24 flex-shrink-0 text-right text-[10px] text-gray-500">
                    {{ formatUsage(subscription.hourly_usage_usd ?? 0, subscription.group.hourly_limit_usd) }}
                  </span>
                </div>
...
function getMaxUsagePercentage(sub: UserSubscription): number {
  ...
  if (sub.group?.hourly_limit_usd) {
    percentages.push(((sub.hourly_usage_usd || 0) / sub.group.hourly_limit_usd) * 100)
  }
