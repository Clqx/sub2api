...
| 恢复协议跨版本兼容 | 新平台 Store + PostgreSQL migration | Phase 2-H 兼容门禁已接入 | 011 仅接管升级前已有的 009 七字段 Seat snapshot，新 operation 只接受 typed 16-key；旧审计不改写，mixed/alias/hash/字段/内嵌 Seat 漂移失败关闭；仍禁止滚动重叠与降级 |
