...
- ntfy and configured Webhooks are outbound notification destinations and receive redacted incident content only.
...
| Credential disclosure in API/log/backup | Write-only API, centralized redaction, encrypted application columns, and restricted backup handling | API snapshots and log/redaction tests; backup/restore rehearsal is release-gated | Compromise of the running process/master secret exposes active secrets; current backup recovery has not been rehearsed. |
| Master-key loss or rotation failure | One externally supplied master secret (minimum length 16) is SHA-256-derived into a Fernet key | Configuration and encryption round-trip tests | Startup validates presence/length only; a wrong but well-formed secret fails when existing ciphertext is decrypted. There is no versioned envelope, KMS adapter, tested rotation, or escrow/recovery workflow yet. |
