...
export const api = {
  ...
  updateCostRoutingPolicy: (targetId:string, body:{enabled:boolean;mode:'recommend'|'execute';probe_interval_seconds:number;priority_scale:number;unhealthy_priority:number;minimum_priority:number;quality_bindings:Record<string,string[]>;confirm_side_effects:boolean}) => request<CostRoutingPolicy>(`/targets/${targetId}/cost-routing-policy`,{method:'PUT',body:JSON.stringify(body)}),
