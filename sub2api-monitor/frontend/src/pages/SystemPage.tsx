...
export function SystemPage() {
  ...
  return <><div className="page-title"><div><h1>系统</h1><p>Hub API、Worker 与数据库运行诊断</p></div></div>{q.isError ? <ErrorState error={q.error}/> : <section className="system-grid">
    ...
    <SystemItem icon={ServerCog} label="采集 Worker" value={q.data ? (q.data.worker_stalled_loops.length ? '关键循环停滞' : q.data.worker_stale ? '心跳过期' : '运行中') : '读取中'} ok={q.data ? !q.data.worker_stale : undefined} detail={q.data?.worker_stalled_loops.length ? q.data.worker_stalled_loops.join(' / ') : q.data?.worker_last_seen_at ? new Date(q.data.worker_last_seen_at).toLocaleString('zh-CN') : '尚无心跳'}/>
