...
管理员在 `/admin` 的“商品”页面维护中英文名称与说明、SKU、展示价格、兑换权益、外部购买链接和上架状态。用户兑换页会根据 Sub2API 传入的 `lang` 展示对应语言；英文内容未填写时回退到中文。只有“已上架”商品会出现在用户兑换页，也只有已上架商品可以生成关联兑换码。

支付流程不在本服务内处理。“前往购买”按钮仅跳转到管理员配置的 HTTP 或 HTTPS 地址；未配置地址时按钮保持禁用。管理员可在“兑换码”页面选择商品后批量生成兑换码，也可以继续使用自定义权益模式。

生成关联兑换码时，余额金额或订阅分组和天数会复制到兑换码中作为权益快照。之后修改商品价格或权益不会改变已经生成的兑换码；商品不再销售时应改为“归档”，历史兑换记录仍保留商品关联。
