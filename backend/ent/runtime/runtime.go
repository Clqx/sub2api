...
func init() {
	...
	// userDescPrincipalType is the schema descriptor for principal_type field.
	userDescPrincipalType := userFields[2].Descriptor()
	// user.DefaultPrincipalType holds the default value on creation for the principal_type field.
	user.DefaultPrincipalType = userDescPrincipalType.Default.(string)
	// user.PrincipalTypeValidator is a validator for the "principal_type" field. It is called by the builders before save.
	user.PrincipalTypeValidator = func() func(string) error {
		validators := userDescPrincipalType.Validators
		fns := [...]func(string) error{
			validators[0].(func(string) error),
			validators[1].(func(string) error),
		}
		return func(principal_type string) error {
			for _, fn := range fns {
				if err := fn(principal_type); err != nil {
					return err
				}
			}
			return nil
	...
	// usersubscriptionDescHourlyUsageUsd is the schema descriptor for hourly_usage_usd field.
	usersubscriptionDescHourlyUsageUsd := usersubscriptionFields[10].Descriptor()
	// usersubscription.DefaultHourlyUsageUsd holds the default value on creation for the hourly_usage_usd field.
	usersubscription.DefaultHourlyUsageUsd = usersubscriptionDescHourlyUsageUsd.Default.(float64)
