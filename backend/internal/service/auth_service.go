...
// VerifyLoginCaptcha validates the active provider and applies persistent IP
// failure tracking to Turnstile login challenges.
func (s *AuthService) VerifyLoginCaptcha(ctx context.Context, proof CaptchaProof, remoteIP, userAgent string) error {
	if s == nil {
		return ErrServiceUnavailable
	}
	if s.settingService == nil {
	...
	return s.turnstileService.VerifyLoginToken(ctx, proof.TurnstileToken, remoteIP, userAgent)
}

// VerifyLoginTurnstile keeps the legacy internal entry point for callers that
// only support Cloudflare Turnstile.
func (s *AuthService) VerifyLoginTurnstile(ctx context.Context, token, remoteIP, userAgent string) error {
	return s.VerifyLoginCaptcha(ctx, CaptchaProof{TurnstileToken: token}, remoteIP, userAgent)
}
