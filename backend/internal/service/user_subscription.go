...
func (s *UserSubscription) NeedsHourlyReset() bool {
	return s.NeedsHourlyResetAt(time.Now())
}

func (s *UserSubscription) NeedsHourlyResetAt(now time.Time) bool {
	if s.HourlyWindowStart == nil {
		return false
	}
	return !now.Before(s.HourlyWindowStart.Add(time.Hour))
}
...
func (s *UserSubscription) canAutomaticallyResetHourlyAt(now time.Time) bool {
	_, ok := s.automaticWindowStartAt(s.HourlyWindowStart, time.Hour, now)
	return ok
}
