...
func normalizeExpiredWindowsAt(subs []UserSubscription, now time.Time) {
	for i := range subs {
		sub := &subs[i]
		if sub.canAutomaticallyResetHourlyAt(now) {
...
func (s *SubscriptionService) checkAndActivateWindowAt(ctx context.Context, sub *UserSubscription, now time.Time) error {
	...
	if sub.HourlyWindowStart == nil && supportsHourly {
		return hourlyRepo.ResetHourlyUsage(ctx, sub.ID, nil, now)
	}
	return nil
}
...
func (s *SubscriptionService) CheckAndResetWindows(ctx context.Context, sub *UserSubscription) error {
	...
	if windowStart, ok := sub.automaticWindowStartAt(sub.HourlyWindowStart, time.Hour, now); ok {
		expectedWindowStart := sub.HourlyWindowStart
		repo, ok := s.userSubRepo.(HourlyUserSubscriptionRepository)
		if !ok {
			return ErrInvalidInput
		}
		if err := repo.ResetHourlyUsage(ctx, sub.ID, expectedWindowStart, windowStart); err != nil {
			return err
		}
		sub.HourlyWindowStart = &windowStart
		sub.HourlyUsageUSD = 0
