...
func TestAPIKeyRepository_GetByKeyForAuth_PreservesHourlyLimit_SQLite(t *testing.T) {
	repo, client := newAPIKeyRepoSQLite(t)
	ctx := context.Background()
	user := mustCreateAPIKeyRepoUser(t, ctx, client, "getbykey-auth-hourly-unit@test.com")

	const hourlyLimit = 10.0
	group, err := client.Group.Create().
		SetName("g-auth-hourly-unit").
		SetPlatform(service.PlatformOpenAI).
		SetStatus(service.StatusActive).
		SetSubscriptionType(service.SubscriptionTypeSubscription).
		SetRateMultiplier(1).
		SetHourlyLimitUsd(hourlyLimit).
		Save(ctx)
	require.NoError(t, err)

	key := &service.APIKey{
		UserID:  user.ID,
		Key:     "sk-getbykey-auth-hourly-unit",
		Name:    "Hourly Key Unit",
		GroupID: &group.ID,
		Status:  service.StatusActive,
	}
	require.NoError(t, repo.Create(ctx, key))
	...
	require.NoError(t, err)
	require.NotNil(t, got.Group)
	require.NotNil(t, got.Group.HourlyLimitUSD)
