...
func SetupRouter(
	...
	trustedPoolAuth *service.TrustedPoolAuthService,
	trustedPoolIntegration *service.TrustedPoolIntegrationService,
) *gin.Engine {
	middleware2.SetIngressRejectRecorder(opsService)
	// 缓存可信自定义页面的 origin，用于动态注入 CSP frame-src 和 connect-src。
	...
	// 注册路由
	registerRoutes(r, handlers, jwtAuth, optionalJWTAuth, adminAuth, apiKeyAuth, auditLog, stepUpAuth, apiKeyService, subscriptionService, opsService, settingService, compositeResolver, cfg, redisClient, trustedPoolAuth, trustedPoolIntegration)
...
func registerRoutes(
	...
	routes.RegisterGatewayRoutes(r, h, apiKeyAuth, apiKeyService, subscriptionService, opsService, settingService, compositeResolver, cfg, trustedPoolIntegration)
	routes.RegisterPaymentRoutes(v1, h.Payment, h.PaymentWebhook, h.Admin.Payment, jwtAuth, adminAuth, auditLog, settingService, panelRateLimiter)
	routes.RegisterTrustedPoolRoutes(v1, h.TrustedPool, trustedPoolAuth)
