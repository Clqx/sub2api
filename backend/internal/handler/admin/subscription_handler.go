...
// ResetQuota resets one or more usage windows for a subscription.
...
func (h *SubscriptionHandler) ResetQuota(c *gin.Context) {
	...
	if !req.Hourly && !req.Daily && !req.Weekly && !req.Monthly {
		response.BadRequest(c, "At least one usage window must be selected")
		return
	}
	sub, err := h.subscriptionService.AdminResetQuota(c.Request.Context(), subscriptionID, req.Hourly, req.Daily, req.Weekly, req.Monthly)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
